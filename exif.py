...
class MakerNoteTags:
	...
	if verbose_opt: logging.debug('%d tags', num_entries)
...
def parse_tag(tiff, mode, value_map, tag_map):
	...
	if verbose_opt > 1: logging.debug("%6s %3d", type.name, count),
	...
	if verbose_opt > 1: logging.debug("@%03x :", tag_offset),
    else:
	if verbose_opt > 1: logging.debug("     :"),
	...
	if verbose_opt:
	    if value_map.has_key(tag.name): logging.debug(val),
...
def parse_ifd(tiff, mode, offset, value_map):
    ...
    num_entries = tiff.short()
    if verbose_opt > 1:
	logging.debug("%30s:        %3d @%03x", "IFD", num_entries, offset)
    for field in range(0, num_entries):
    ...
    if tiff.short() == 42:
        ...
        while offset > 0:
            ...
            for field in range(0, num_entries):
                ...
                length = tiff.long()
                valoff = tiff.long()
                #logging.debug TAG_MAP[ tag_id ].name, length
...
def parse(file, verbose = 0, mode = 0):
	...
	if data[0:4] == '\377\330\377\341' and data[6:10] == 'Exif':
	    ...
	    if verbose > 1:
                logging.debug('%30s:  %d', "EXIF header length",length)
